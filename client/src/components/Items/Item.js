...
export default ({ itemName, category, storage, datePurchased, exp, onList, id, addToPantry, removeFromPantry, ageState, handleEditSubmit, toggleModal, showModal }) => {
  console.log("handleEditSubmit:", handleEditSubmit);
  ...
  return showModal ? <ItemEdit id={id} itemName={itemName} category={category} datePurchased={datePurchased} exp={exp} storage={storage} showModal={toggleModal} show={showModal} handleEditSubmit={handleEditSubmit} />
    : (
