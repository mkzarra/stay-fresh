...
export default ({ itemName, category, storage, datePurchased, exp, onList, id, addToPantry, removeFromPantry, ageState, handleEditSubmit, toggleModal, showModal }) => {
  ...
  return showModal === id && id ? <ItemEdit id={id} itemName={itemName} category={category} datePurchased={datePurchased} exp={exp} storage={storage} showModal={toggleModal} show={showModal === id} handleEditSubmit={handleEditSubmit} />
    : (
